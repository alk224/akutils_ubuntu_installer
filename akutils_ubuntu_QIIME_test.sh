...
db_format.sh $testdir/gg_database/97_rep_set_1000.fasta $testdir/gg_database/97_taxonomy_1000.txt $testdir/resources/primers_515F-806R.txt 250 $testdir/db_format_out 1>$testdir/std_out 2>$testdir/std_err || true
wait
echo "
***** dbformat.sh std_out:
" >> $log
...
PhiX_filtering_workflow.sh $testdir/PhiX_filtering_out $testdir/map.test.txt $testdir/strip_primers_out/index1.noprimers.fastq $testdir/strip_primers_out/read1.noprimers.fastq $testdir/strip_primers_out/read2.noprimers.fastq 1>$testdir/std_out 2>$testdir/std_err 2>&1 || true
wait
echo "
...
Single_indexed_fqjoin_workflow.sh $testdir/PhiX_filtering_out/index.phixfiltered.fastq $testdir/PhiX_filtering_out/read1.phixfiltered.fastq $testdir/PhiX_filtering_out/read2.phixfiltered.fastq 12 -m 30 -p 30 1>$testdir/std_out 2>$testdir/std_err || true
wait
echo "
...
otu_picking_workflow.sh ./ 16S 1>$testdir/std_out 2>$testdir/std_err || true
wait
cd $homedir
echo "
...
cd $testdir/otu_picking_workflow_out
align_and_tree_workflow.sh swarm_otus_d1/ 16S 1>$testdir/std_out 2>$testdir/std_err || true
wait
cd $homedir
echo "
...
cd $testdir/otu_picking_workflow_out
cdiv_graphs_and_stats_workflow.sh swarm_otus_d1/OTU_tables_blast_tax/03_table_hdf5.biom map.test.txt Community $cpus 1>$testdir/std_out 2>$testdir/std_err || true
wait
cd $homedir
echo "
