...
sudo -u $userid git clone https://github.com/alk224/akutils.git
fi
if [[ ! -d $homedir/vsearch ]]; then
sudo -u $userid git clone https://github.com/torognes/vsearch.git
fi
if [[ ! -d $homedir/bamtools ]]; then
sudo -u $userid git clone git://github.com/pezmaster31/bamtools.git
fi
wait
if [[ ! -d $homedir/QIIME_test_data_16S ]]; then
sudo -u $userid git clone https://github.com/alk224/QIIME_test_data_16S.git
fi
wait
if [[ ! -d $homedir/QIIME_databases ]]; then
sudo -u $userid git clone https://github.com/alk224/QIIME_databases.git
fi
...
sudo -u $userid git clone https://github.com/qiime/qiime-deploy.git
fi
if [[ ! -d $homedir/qiime-deploy-conf ]]; then
sudo -u $userid git clone git://github.com/qiime/qiime-deploy-conf.git
fi
wait
cd qiime-deploy/
